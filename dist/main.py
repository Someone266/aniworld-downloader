...
Ax='Enter the name of the anime you want to search for'
Aw='No internet connection'
Av='There is a program update available'
Au='Downloading assets...'
At='You are up to date'
As='success'
Ar='message'
Aq='Something went wrong with the update check'
Ap='q. Quit'
Ao='0. Settings'
An='7. Get anime info'
Am='6. Setup anime folder'
Al='5. Clean up'
Ak='4. Serve content'
Aj='3. Continue Download'
Ai='2. Manual Download'
Ah='1. Download Assistant'
Ag='Checking for updates...'
Af='aniworld.to'
Ae='Episoden:'
Ad='animeSeason'
Ac=enumerate
...
from bs4 import BeautifulSoup as Ay
from yt_dlp import YoutubeDL as Az
import shutil as h
def BJ():
	B=K.argv
	try:B[1]
...
def i(URL,path=D,Season=D,Episode=D):
	...
	c=P.get(T);Z=Ay(c.content,q);a=Z.find(J).text;U=a.index('Watch ')+6;B=a[U:];V=B.index(' - VOE');B=B[:V];B=B.replace(l,'_');A(B)
	...
	try:
		...
		try:
			...
			with E(F+X,r)as W:W.write(O+I)
			f={'outtmpl':B}
			with Az(f)as i:i.download(O)
			B0()
		except Ab:A('Could not find downloadable URL. Voe might have change their site. Check that you are running the latest version of voe-dl, and if so file an issue on GitHub.');quit()
	A('Downloaded: '+B)
	if K is not D and N is not D:C.makedirs(f"{F}/video/{K}",exist_ok=Q);h.move(f"{B}",f"{F}/video/{K}/{N}.mp4");A(f"Moved to {F}/video/{K}/{N}.mp4")
	A(I)
def B0(animePath=D):
...
def X(url):
	...
	for b in k(1,T(B)+1):F[b]=B2(N,b)
	F=F;e=sum(F.values());X=C.find(L,class_='seriesCoverBox').find('noscript').find('img').get('src');X=A2+X;j=C.find(r,class_='trailerButton').get(AL);Y=C.find_all(r,itemprop='genre');Y=[A.text for A in Y];m=C.find(A1,class_='seriesProducer').text;n=C.find(L,class_=A3).get('data-fsk');G=C.find(r,class_='imdb-link')
	if G is not D:G=G.get(AL)
	else:G=M
	A(Q+' ('+K+' - '+O+') - '+H(e)+' episodes - '+H(B)+' seasons');o={J:E,c:i,Ad:{'year':T(K[:4]),'season':K[5:]},U:Q,W:F,d:e,V:B,R:K,S:O,s:X,AM:j,f:Y,AN:m,A3:n,AO:G,AP:N};return o
def o(info):
	...
	D=D.replace('%title%',B[J]);D=D.replace('%description%',B[c]);D=D.replace('%startDate%',B[R]);D=D.replace('%endDate%',B[S]);D=D.replace('%status%',B[U]);D=D.replace('%totalEpisodes%',H(B[d]));D=D.replace('%seasons%',H(B[V]));D=D.replace('%picture%',f"image.{L}");D=D.replace('%trailer%',B[AM]);D=D.replace('%producer%',B[AN]);D=D.replace('%fsk%',B[A3]);D=D.replace('%imdb%',B[AO]);D=D.replace('%tags%',', '.join(B[f]));D=D.encode('utf-8')
...
def AC(url,season,episodes=D,info=D):
	...
	if B is D:H=P.get(F);J=H.text;K=w.BeautifulSoup(J,q);B=K.find(A1,text=Ae).parent.parent.parent;B=B.find_all('li');B=Z(B)-1
...
def AE(url,season,episode,info=D):
	...
	B=B.parent;B=B.get(AL);B=A2+B;Q=B1(B)
...
def B1(url):
	...
	if B.status_code!=302 and B.status_code!=301:A('No redirect found');return
		...
		if Af in C:A('Something went wrong with the redirect');return
		else:return C
def B2(url,season):B=url;B=B+f"/staffel-{season}";C=P.get(B);D=C.text;E=w.BeautifulSoup(D,q);A=E.find(A1,text=Ae).parent.parent.parent;A=A.find_all('li');A=Z(A)-1;return A
def B3(season,episode,info=D):
	F=episode;I=season;C=info;N();L()
	if C is D:
		with E(AQ,O)as K:P=G.load(K)
	else:
		B=f"{C[J]} ({C[R]}-{C[S]})";B=g.sub(t,M,B);B=f"anime/{B}"
		with E(B+AR,O)as K:P=G.load(K)
	A(f"Downloading season {I} episode {F}...");Q=P[H(I)][H(F)];A(Q);i(Q,B,I,F)
def AF(season,info=D):
	F=info;C=season
	if F is D:
		with E(AQ,O)as Q:K=G.load(Q)
	else:
		B=f"{F[J]} ({F[R]}-{F[S]})";B=g.sub(t,M,B);B=f"anime/{B}"
		with E(B+AR,O)as Q:K=G.load(Q)
	for P in K[H(C)]:U=K[H(C)][P];N();L();A(f"Downloading season {C} episode {P}...");AD(T(P)/Z(K[H(C)]));A(I);i(U,B,C,P)
def B4(season,start,end,info=D):
	K=start;P=season;C=info
	if C is D:
		with E(AQ,O)as Q:T=G.load(Q)
	else:
		B=f"{C[J]} ({C[R]}-{C[S]})";B=g.sub(t,M,B);B=f"anime/{B}"
		with E(B+AR,O)as Q:T=G.load(Q)
...
def AG(query,data):
	...
	for(V,K)in Ac(C):K[J]=K[J].replace('<em>',M).replace('</em>',M);A(V+1,B.OK+K[J]+B.ENDC);A(K[c]);A('---')
...
def AV():
	B=3333;D=http.server.SimpleHTTPRequestHandler;E=C.path.join(C.path.dirname(__file__),AS);C.chdir(E)
...
def AH():
	...
	A('Rebuilding index.json...');b=[A for A in C.listdir(AS)if C.path.isdir(C.path.join(AS,A))];Z=[]
	for H in b:
		...
		e=[J,c,Ad,U,W,d,V,R,S,s,AM,f,AN,A3,AO]
		if not all(A in D for A in e):A('Rebuilding info.json for:',D[J]);D=X(D[AP]);o(D)
...
def AI(url):
	...
	if Af not in B:A(A5);return A0
	if D in B:B=B.split(D)[0]
	if C in B:B=B.split(C)[0]
	return B
def B5():
	if not C.path.exists(u):B7();return
...
def B7():
	...
	B8()
def B8():
	E='updateNotes';G='Update notes:';N();L();A(B.PRIMARY+'Step 1: Checking for updates and downloading assets'+B.ENDC);A('We promise, it will be quick');C=AY()
	...
	if C[U]=='error':A(B.WARNING+Aq+B.ENDC);A(AT,C[Ar]);A('Please try again later\n');A(B.SECONDARY+A9+B.ENDC);F();p()
	elif C[U]==As:A(At);A(B.SECONDARY+n+B.ENDC);F();AJ();return
	else:
		if C[AA][e]:A(B.OK+Au+B.ENDC);A(G);A(C[E][e]+I);y(e,C[AB][e])
		if C[AA][b]:A(B.OK+Av+B.ENDC);A(G);A(C[E][b]+I);y(b,C[AB][b])
		else:A('No program update available')
		A(B.SECONDARY+n+B.ENDC);F();AJ();return
def AJ():N();L();A(B.PRIMARY+'Step 2: Configuring the program'+B.ENDC);A('We need to configure the program before we can continue');A('It is very simple, just answer a few questions');A(B.SECONDARY+n+B.ENDC);F();AX()
def AX():
	...
	if D=='1':AW(C,'archive')
	elif D=='2':AW(C,'download')
	else:A('Invalid option, try again');AX();return
	B9()
def B9():
	...
	if C=='i':BA()
	Y()
def BA():N();L();A(B.PRIMARY+'Instructions'+B.ENDC);A(Ah);A('This option will guide you through the download process, you just need to enter the url of the anime');A(Ai);A('This option will download everything from the given url');A(Aj);A('This option will continue the download from the last download');A(Ak);A('This option will start a http server with document root to the anime folder');A(Al);A("This option will delete non finished downloads, after deleting you can't resume the download");A(Am);A('This option will setup the anime folder with the given url');A(An);A('This option will get the anime info from the given url');A(Ao);A('This option will show the settings');A(Ap);A(I);A(n);F();Y()
def BB():
	B5();N();L();A('Loading...');B=AY()
	if B is D:A('No internet connection, skipping update check');return
	if B[U]=='error':A(Aq);A(AT,B[Ar]);Q.sleep(2);return
	elif B[U]==As:A(At);return
	else:
		if B[AA][e]:A(Au);y(e,B[AB][e])
		if B[AA][b]:A(Av);y(b,B[AB][b])
def AY():
	D='./assets/.version';A(Ag,end=M)
	try:P.get('https://api.jm26.net/status.txt')
	except P.exceptions.ConnectionError:A(' Skipped');A(B.WARNING+Aw+B.ENDC);Q.sleep(2);return
...
def y(type,hash):
	...
	try:L=P.get(I)
	except P.exceptions.ConnectionError:A(B.WARNING+Aw+B.ENDC);A(N);A(A9);F();p()
	if L.headers['Content-Type']=='application/json':A(B.WARNING+'Something went wrong with the download'+B.ENDC);A(AT,L.text);A(N);A(A9);F();p()
...
def Aa():
	...
	if E=='S'or E=='s':N();L();A(Ax);H=F();C=AU(H)
...
def AK():return BC
def BD(url):j();A=X(url);return A
def BE(url):j();A=X(url);o(A)
def BF(url,season,info=D):
	A=season;j();B=D
	if info is not D:B=info[W][A]
	C=AC(url,A,B);return C
def BG(url,season,episode,info=D):
	C=season;A=info;B=episode;j()
	if A is not D:B=A[W][C][B]
	E=AE(url,C,B,A);return E
def BH():N();L();A(Ax);B=F();return AU(B)
def L():
	A(I);C=AK()
	if x():C=C+' (Windows)'
...
def BI():
	...
	C=K.argv[1]
	if C=='guideUpdateFinished':B6();return
	BB();N();L();A(I)
	if C=='getinfo':E=K.argv[3];A(BD(E))
	elif C=='run':E=K.argv[2];AZ(E)
	elif C=='ui':Y()
	elif C==O:BH();return
	elif C==M:AV()
	elif C==J:AH()
	elif C=='setup':E=K.argv[3];BE(E)
	elif C=='auto':Aa()
	elif C=='getseason':
